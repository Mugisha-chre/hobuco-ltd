...
document.addEventListener("DOMContentLoaded", function () {
  ...
  if (faqItems.length === 0) {
    console.warn("⚠️ No .faq-item elements found. Make sure FAQ HTML is loaded before this script runs.");
    return;
  }
  ...
  faqItems.forEach((item) => {
    ...
    if (!question || !answer) {
      console.warn("⚠️ .faq-question or .faq-answer not found in one of the .faq-item elements.");
      return;
    }
...
const startYear = 2016;
  ...
  document.getElementById("yearsCount").textContent = `${yearsAgo} years ago`;
